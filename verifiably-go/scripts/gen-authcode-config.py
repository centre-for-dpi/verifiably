...
# --- per-schema extraction VIEW over claims jsonb (Flow B emits one per schema) ---
view_cols = ",\n".join("  claims->>'%s' AS \"%s\"" % (f, f) for f in FIELDS)
vc_subject_view = ("CREATE OR REPLACE VIEW certify.vc_subject_person AS\n"
...
out_sql = (header + ddl + "\n" + vc_subject_ddl + vc_subject_view + "\n" + insert
...
query = ('select %s from certify.vc_subject_person where individual_id=:id'
