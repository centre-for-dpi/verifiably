...
| `deploy/mimoto-inji/mimoto-issuers-config.json` | `vca setup` writes it (P6-I7h). Its structure is the "Mimoto Issuers Configuration" section of https://raw.githubusercontent.com/mosip/inji-web/v0.16.0/docker-compose/README.md and https://raw.githubusercontent.com/mosip/inji-certify/v0.14.0/docker-compose/docker-compose-injistack/config/mimoto-issuers-config.json | One issuer: `inji-certify-esignet` behind the second server of `inji-certify-nginx`. The redirect page and the token proxy sit on the address of Inji Web, and the audience is the token endpoint of the eSignet login page. |
| `deploy/mimoto-inji/mimoto-trusted-verifiers.json` | `vca setup` writes it (P6-I7h) in the structure of https://raw.githubusercontent.com/mosip/mimoto/v0.21.0/docker-compose/config/mimoto-trusted-verifiers.json | The Inji Verify of the stack on the addresses of the deployment. |
