...
parse, validate and build data. They do not serve HTTP, store data or
keep global state. Network fetches are injected as functions. Caches are
...
- Legacy test cases are kept as regression tests. Each is marked `Regression:`.

## Packages

| Package | Purpose | Standard |
|---|---|---|
| `core/jose` | Compact JWS sign (ES256, EdDSA) and verify (plus RS256), JWK and JWKS parse, `kid` selection, thumbprints. Wraps go-jose v4. | [RFC 7515](https://www.rfc-editor.org/rfc/rfc7515), [RFC 7517](https://www.rfc-editor.org/rfc/rfc7517), [RFC 7518](https://www.rfc-editor.org/rfc/rfc7518), [RFC 7638](https://www.rfc-editor.org/rfc/rfc7638) |
| `core/statuslist/bitstring` | Bitstring status list: allocate, set, get, gzip and multibase base64url encode and decode, minimum 131072 entries, purposes revocation, suspension and message, credential and entry builders. | [W3C Bitstring Status List v1.0](https://www.w3.org/TR/vc-bitstring-status-list/) |
...
| `core/sdjwt` | SD-JWT parse, disclosure digests, disclosure resolution for objects and arrays, key binding JWT build and verify (`cnf`, `typ`, `aud`, `nonce`, `iat`, `sd_hash`). | [RFC 9901](https://www.rfc-editor.org/rfc/rfc9901) |
...
| `core/trust` | Trust entry model, pure lookup, signed trust list JWT build and verify (ES256, EdDSA). | [RFC 7519](https://www.rfc-editor.org/rfc/rfc7519) |
...
| `core/oidc` | PKCE S256 generation and verification, discovery document parsing and endpoint rebasing, authorisation URL, ID token verification against a JWKS (RS256, ES256). | [RFC 7636](https://www.rfc-editor.org/rfc/rfc7636), [OpenID Connect Core 1.0](https://openid.net/specs/openid-connect-core-1_0.html), [OpenID Connect Discovery 1.0](https://openid.net/specs/openid-connect-discovery-1_0.html) |
...
Both modules are hosted on GitHub. On a restricted network
...
- `core/jose` replaces the hand-rolled JWS in `internal/statuslist/jws.go`,
  `internal/trust/jwt.go` and `internal/jose`. The trust registry HS256 mode
  is dropped. A trust list JWT carries `typ: trust-list+jwt`.
- `core/sdjwt` rejects a disclosure that matches no `_sd` digest. The legacy
  ...
  The key binding JWT is now verified. The legacy code did not verify it.
...
- File backed key storage (`selfsigned.go`, `ldproof.go`) and the JSON-LD
  Data Integrity proof are not lifted. Key storage is a service concern.
  The `Ed25519Signature2020` proof needs `json-gold` and bundled contexts
  and is left for the status-bitstring service.
