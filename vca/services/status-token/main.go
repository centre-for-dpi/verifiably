...
// Command status-token serves vca.status.v1.StatusService and the
// public IETF Token Status List tokens (ADR-019).
...
func run(ctx context.Context, args []string, getenv func(string) string, stdout, stderr io.Writer) int {
	flags := flag.NewFlagSet("status-token", flag.ContinueOnError)
