...
// Package app wires the status-bitstring service from its
// configuration: the store, the per issuer key rings, the securer, the
// list manager, the Connect handler, and the public HTTP endpoints.
package app
...
func Build(ctx context.Context, cfg config.Config, deps Deps) (*App, error) {
	...
	sec, err := securer.New(cfg.SecuringMethod)
	if err != nil {
		return nil, err
	}
	...
	manager, err := lists.Open(ctx, lists.Options{
		...
		Size: cfg.ListSize, TTL: cfg.ListTTL, Now: deps.Now,
	})
	if err != nil {
		return nil, err
	}
	...
	deps.Log.Info("status-bitstring ready",
		"base_url", cfg.BaseURL, "method", cfg.SecuringMethod,
