...
// Command status-bitstring serves vca.status.v1.StatusService and the
// public W3C Bitstring Status List credentials (ADR-018).
...
func run(ctx context.Context, args []string, getenv func(string) string, stdout, stderr io.Writer) int {
	flags := flag.NewFlagSet("status-bitstring", flag.ContinueOnError)
...
func serveOrProbe(ctx context.Context, healthcheck bool, getenv func(string) string, stdout io.Writer) error {
	...
	a, err := app.Build(ctx, cfg, app.Deps{Log: log})
	if err != nil {
		return err
	}
