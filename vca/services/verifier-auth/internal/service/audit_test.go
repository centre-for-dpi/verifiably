...
func TestSignInWritesAuditEvent(t *testing.T) {
	...
	session := f.signIn(t)
	start, err := f.client.LoginStart(ctx, connect.NewRequest(&verifierauthv1.LoginStartRequest{ProviderId: "idp"}))
	if err != nil {
		t.Fatalf("LoginStart: %v", err)
	}
	if _, cerr := f.client.LoginCallback(ctx, connect.NewRequest(&verifierauthv1.LoginCallbackRequest{State: start.Msg.GetState(), Error: "access_denied"})); cerr == nil {
	...
	if _, cerr := f.client.LoginCallback(ctx, connect.NewRequest(&verifierauthv1.LoginCallbackRequest{State: u.Query().Get("state"), Code: u.Query().Get("code")})); cerr != nil {
		t.Fatalf("register callback: %v", cerr)
	}
	if _, lerr := f.client.Logout(ctx, connect.NewRequest(&verifierauthv1.LogoutRequest{SessionToken: session})); lerr != nil {
	...
	for _, e := range events {
		...
		if e.GetSourceService() != "verifier-auth" || e.GetRequestId() == "" || e.GetTime() == nil {
			t.Errorf("event %+v", e)
		}
	}
	if strings.Join(got, "\n") != strings.Join(want, "\n") {
...
// with the verifier-admin role, is refused.
func TestAuditQueryNeedsAdmin(t *testing.T) {
	f := newFixture(t)
	f.idp.Claims["realm_access"] = map[string]any{"roles": []any{"verifier-admin"}}
	ctx := context.Background()
	staff := f.signIn(t)
	// The staff session is a verifier admin: it opens the provider RPCs.
	...
	if _, err := providers.ListAuthProviders(ctx, connect.NewRequest(&adminv1.ListAuthProvidersRequest{})); err != nil {
		t.Fatalf("the verifier admin session: %v", err)
	}
