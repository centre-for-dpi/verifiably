...
func (s *Service) Admin() Admin {
	return Admin{
		AdminProviders: oidcflow.AdminProviders{Registry: s.d.Providers, Authorize: s.AdminAuthorizer(), Roles: []string{"verifier"}, InternalAuthority: s.cfg.ProviderInternalAuthority},
		svc:            s,
	}
}

// MachineRoles maps the deployment roles of an API key to verifier
// roles: admin gives verifier-admin, verifier gives verifier-operator,
// and every other role gives verifier-viewer.
