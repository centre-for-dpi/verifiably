...
func TestFromEnvDefaults(t *testing.T) {
	c, err := config.FromEnv(env(map[string]string{"VCA_PUBLIC_URL": "https://verifier.example/"}))
	if err != nil {
		t.Fatal(err)
	}
	if c.Listen != ":8081" || c.PublicBaseURL != "https://verifier.example" || c.RedirectURI != "https://verifier.example/auth/callback" {
		t.Fatalf("%+v", c)
	}
	if c.SessionTTL != 15*time.Minute || c.MachineTokenTTL != time.Hour || c.TenantID != "default" || c.CookieName != "vca_verifier_session" || c.InsecureCookie {
		t.Fatalf("%+v", c)
	}
...
func TestFromEnvValues(t *testing.T) {
	c, err := config.FromEnv(env(map[string]string{
		...
		"VCA_VERIFIER_AUTH_LISTEN":            ":9000",
		"VCA_OIDC_REDIRECT_URI":               "http://localhost:8081/callback",
		"VCA_VERIFIER_AUTH_SESSION_TTL":       "5m",
		"VCA_VERIFIER_AUTH_MACHINE_TOKEN_TTL": "30m",
		"VCA_VERIFIER_AUTH_INSECURE_COOKIE":   "true",
		"VCA_VERIFIER_AUTH_LOGOUT_REDIRECT":   "/bye",
		...
		"VCA_VERIFIER_AUTH_LANDING_URL":       "https://vca.example/",
...
func TestFromEnvErrors(t *testing.T) {
	cases := []map[string]string{
		{},
		{"VCA_PUBLIC_URL": "verifier.example"},
		{"VCA_PUBLIC_URL": "ftp://x"},
		{"VCA_PUBLIC_URL": "https://x", "VCA_VERIFIER_AUTH_SESSION_TTL": "soon"},
		{"VCA_PUBLIC_URL": "https://x", "VCA_VERIFIER_AUTH_SESSION_TTL": "-1m"},
		{"VCA_PUBLIC_URL": "https://x", "VCA_VERIFIER_AUTH_MACHINE_TOKEN_TTL": "x"},
		{"VCA_PUBLIC_URL": "https://x", "VCA_VERIFIER_AUTH_INSECURE_COOKIE": "maybe"},
		{"VCA_PUBLIC_URL": "https://x", "VCA_VERIFIER_AUTH_LOGOUT_REDIRECT": "https://evil"},
	}
	for i, m := range cases {
