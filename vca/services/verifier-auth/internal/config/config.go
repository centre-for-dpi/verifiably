...
// Package config reads the verifier-auth settings from the environment
// with the shared config package (ADR-036, ADR-012).
package config
...
type settings struct {
	...
	CookieName string `env:"COOKIE_NAME" default:"vca_verifier_session"`
...
func (c Config) normalize() (Config, error) {
	...
	if c.LogoutRedirect != "" && !strings.HasPrefix(c.LogoutRedirect, "/") {
		return Config{}, errors.New("config: VCA_VERIFIER_AUTH_LOGOUT_REDIRECT must be a relative path")
	}
	return c, nil
}
