...
func TestRegistry(t *testing.T) {
	...
	if _, _, serr := reg.Create("", "t", []string{"verifier-admin"}, nil); !errors.Is(serr, clients.ErrInvalid) {
		t.Fatal(serr)
	}
	if _, _, serr := reg.Create("x", "t", nil, nil); !errors.Is(serr, clients.ErrInvalid) {
		t.Fatal(serr)
	}
	c, secret, err := reg.Create("robot", "t1", []string{"verifier-operator"}, nil)
	...
	if err != nil || got.ID != c.ID || got.Roles[0] != "verifier-operator" {
		t.Fatalf("auth: %v", err)
	}
	...
	c2, s2, verr := reg.Create("temp", "t2", []string{"verifier-viewer"}, &exp)
	if verr != nil {
	...
	store.fail = true
	if _, _, err := reg.Create("y", "t", []string{"verifier-admin"}, nil); err == nil {
