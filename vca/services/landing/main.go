...
// Command landing serves the front door of a VCA deployment: what VCA
// is, the stacks that run, the role picker, and the role intro pages
// (ADR-033). Run with -healthcheck to probe /readyz and exit.
package main
...
func run(ctx context.Context, args []string, getenv func(string) string, stdout, stderr io.Writer) int {
	flags := flag.NewFlagSet("landing", flag.ContinueOnError)
