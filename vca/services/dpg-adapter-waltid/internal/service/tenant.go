...
// tenantMessage says why the tenant service answers Unimplemented. The community stack has no multi tenancy.
// The capability answer lists no FEATURE_MULTI_TENANCY, so the admin
// pages offer no tenancy on this stack (ADR-037 decision 3).
const tenantMessage = "the walt.id Community Stack keeps no tenants; one deployment serves one organisation"
