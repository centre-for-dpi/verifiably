...
// Command dpg-adapter-waltid serves the vca.backend.v1 services against
// the walt.id Community Stack (ADR-002 decision 2). Run it with
...
func run(ctx context.Context, args []string, getenv func(string) string, stdout, stderr io.Writer) int {
	flags := flag.NewFlagSet("dpg-adapter-waltid", flag.ContinueOnError)
