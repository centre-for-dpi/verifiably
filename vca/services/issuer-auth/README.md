...
`issuer-auth` logs issuer staff in with OpenID Connect and issues session tokens for the issuer role.
...
| `/vca.issuerauth.v1.IssuerAuthService/*` | POST | The Connect RPCs of the contract. |
...
The service maps provider claims to `issuer-admin`, `issuer-operator`, and `issuer-viewer` (ADR-012 decision 3). The default mapping reads the claim at `realm_access.roles` and expects the same three names. An admin changes the mapping per provider with `SetRoleMapping`. A subject with no matching claim cannot log in.

A machine client gets roles from its API key: the `admin` deployment role gives `issuer-admin`, the `issuer` role gives `issuer-operator`, and every other role gives `issuer-viewer`.
...
```sh
vca setup --role issuer --dpg <dpg>
vca deploy --role issuer --dpg <dpg>
```

Now:

```sh
cd services/issuer-auth
VCA_PUBLIC_URL=http://localhost:8081 VCA_ISSUER_AUTH_INSECURE_COOKIE=true go run .
```
...
|---|---|---|
| `VCA_ISSUER_AUTH_LISTEN` | The address the service listens on. | `:8081` |
...
| `VCA_ISSUER_AUTH_SESSION_TTL` | The lifetime of a session JWT. | `15m` |
| `VCA_ISSUER_AUTH_MACHINE_TOKEN_TTL` | The lifetime of a client credentials JWT. | `1h` |
| `VCA_ISSUER_AUTH_TENANT_ID` | The tenant of every session. | `default` |
| `VCA_ISSUER_AUTH_ADMIN_TOKEN` | The bearer token the admin service uses for the admin RPCs. | none |
| `VCA_ISSUER_AUTH_ADMIN_JWKS_URL` | The key set of the admin service. An admin session it signed opens the provider RPCs too. | empty: no admin session is accepted |
| `VCA_ISSUER_AUTH_STATE_DIR` | The directory for the persisted documents. Empty keeps them in memory. | none |
...
curl -X POST http://localhost:8081/vca.admin.v1.AdminService/CreateAuthProvider \
  -H "Authorization: Bearer $VCA_ISSUER_AUTH_ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"provider":{"displayName":"Keycloak","discoveryUrl":"https://idp.example/realms/issuer/.well-known/openid-configuration","clientId":"vca-issuer","clientSecret":{"store":"STORE_ENV","name":"KEYCLOAK_SECRET"},"enabled":true}}'
```
