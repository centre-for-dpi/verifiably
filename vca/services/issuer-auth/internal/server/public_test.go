...
// publishes the IssuerAuthService and the AdminService of issuer-auth, so
...
func TestAnonymousRPCsAreRefused(t *testing.T) {
	...
	for service, open := range map[string][]string{
		issuerauthv1connect.IssuerAuthServiceName: {"ListProviders", "LoginStart", "LoginCallback", "Introspect"},
