...
// Package roles maps IdP claims to issuer roles (ADR-012 decision 3).
package roles
...
func Name(r issuerauthv1.IssuerRole) string {
	switch r {
	case issuerauthv1.IssuerRole_ISSUER_ROLE_ADMIN:
		return Admin
	case issuerauthv1.IssuerRole_ISSUER_ROLE_OPERATOR:
		return Operator
	case issuerauthv1.IssuerRole_ISSUER_ROLE_VIEWER:
		return Viewer
	}
	return ""
}
...
func Value(name string) issuerauthv1.IssuerRole {
	switch name {
	case Admin:
		return issuerauthv1.IssuerRole_ISSUER_ROLE_ADMIN
	case Operator:
		return issuerauthv1.IssuerRole_ISSUER_ROLE_OPERATOR
	case Viewer:
		return issuerauthv1.IssuerRole_ISSUER_ROLE_VIEWER
	}
	return issuerauthv1.IssuerRole_ISSUER_ROLE_UNSPECIFIED
}

// Names converts proto roles to token names and drops unspecified ones.
func Names(rs []issuerauthv1.IssuerRole) []string {
	var out []string
	for _, r := range rs {
		if n := Name(r); n != "" {
			out = append(out, n)
...
func Values(names []string) []issuerauthv1.IssuerRole {
	var out []issuerauthv1.IssuerRole
	for _, n := range names {
		if v := Value(n); v != issuerauthv1.IssuerRole_ISSUER_ROLE_UNSPECIFIED {
			out = append(out, v)
		}
	}
	return out
...
// Mapping is the stored form of vca.issuerauth.v1.RoleMapping.
type Mapping struct {
...
func Apply(m Mapping, claims map[string]any) ([]string, error) {
	...
	for _, r := range m.Rules {
		if Value(r.Role) == issuerauthv1.IssuerRole_ISSUER_ROLE_UNSPECIFIED {
			continue
		}
	...
	if len(set) == 0 {
		if Value(m.DefaultRole) == issuerauthv1.IssuerRole_ISSUER_ROLE_UNSPECIFIED {
...
func FromProto(m *issuerauthv1.RoleMapping) Mapping {
...
func ToProto(m Mapping) *issuerauthv1.RoleMapping {
	out := &issuerauthv1.RoleMapping{ClaimPath: m.ClaimPath, DefaultRole: Value(m.DefaultRole)}
	for _, r := range m.Rules {
		out.Rules = append(out.Rules, &issuerauthv1.RoleMapping_Rule{ClaimValue: r.ClaimValue, Role: Value(r.Role)})
	}
	return out
}
...
func (m Mapping) Validate() error {
	...
	for _, r := range m.Rules {
		if r.ClaimValue == "" || Value(r.Role) == issuerauthv1.IssuerRole_ISSUER_ROLE_UNSPECIFIED {
			return oidcflow.ErrInvalidProvider
		}
	}
	return nil
