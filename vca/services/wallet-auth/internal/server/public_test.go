...
// publishes the WalletAuthService and the AdminService of wallet-auth, so
...
func TestAnonymousRPCsAreRefused(t *testing.T) {
	...
	for service, open := range map[string][]string{
		walletauthv1connect.WalletAuthServiceName: {"ListProviders", "LoginStart", "LoginCallback", "Introspect"},
