...
func TestLinkValuesFeedEveryService(t *testing.T) {
	...
	want := map[string]string{
		...
		"VCA_SCHEMABUILDER_AUTH_JWKS_URL":       "http://issuer-waltid-issuer-auth:8081/.well-known/jwks.json",
		"VCA_SCHEMABUILDER_LOGIN_URL":           "https://issuer-waltid.labs.example/auth/",
		"VCA_SCHEMA_AUTH_JWKS_URL":              "http://issuer-waltid-issuer-auth:8081/.well-known/jwks.json",
		"VCA_SCHEMA_LOGIN_URL":                  "https://issuer-waltid.labs.example/auth/",
	...
	for name, value := range map[string]string{
		...
		"VCA_DISCOVERY_AUTH_JWKS_URL":         "http://verifier-credebl-verifier-auth:8081/.well-known/jwks.json",
		"VCA_DISCOVERY_LOGIN_URL":             "https://issuer-waltid.labs.example/auth/",
		"VCA_INGEST_AUTH_JWKS_URL":            "http://verifier-credebl-verifier-auth:8081/.well-known/jwks.json",
...
func TestUIServicesAreTheOnesWithPages(t *testing.T) {
	...
	want := []string{"admin", "data-source", "issuance", "issued-credentials", "issuer-auth", "landing", "schema-builder-ui", "schema-registry", "verifier-auth", "verifier-discovery", "verifier-ingest", "verifier-results", "wallet-auth", "wallet-portal"}
...
func TestPeersReachTheRightServices(t *testing.T) {
	want := map[string]bool{
		...
		"data-source": true, "issued-credentials": true,
	}
	for _, s := range Catalog() {
		has := false
