...
func TestThemeApplyRestartsOnlyUIServices(t *testing.T) {
	...
	wantIssuer := append(ComposeArgs(root, issuer, []string{"restart"}),
		"issuer-waltid-data-source", "issuer-waltid-issuance", "issuer-waltid-issued-credentials", "issuer-waltid-issuer-auth",
	...
	for _, call := range rec.calls {
		...
		if strings.Contains(text, "dpg-adapter") || strings.Contains(text, "status-bitstring") || strings.Contains(text, "trust-registry") {
			t.Errorf("apply restarted a service with no pages: %s", text)
		}
	}
	for _, want := range []string{"docker compose", "restart issuer-waltid-data-source issuer-waltid-issuance issuer-waltid-issued-credentials issuer-waltid-issuer-auth", "restart admin-inji-admin"} {
