...
func TestThemeApplyRestartsOnlyUIServices(t *testing.T) {
	...
	wantIssuer := append(ComposeArgs(root, issuer, []string{"restart"}),
		"issuer-waltid-data-source", "issuer-waltid-issuance", "issuer-waltid-issuer-auth", "issuer-waltid-schema-builder-ui", "issuer-waltid-schema-registry", "vca-landing")
	...
	for _, want := range []string{"docker compose", "restart issuer-waltid-data-source issuer-waltid-issuance issuer-waltid-issuer-auth issuer-waltid-schema-builder-ui", "restart admin-inji-admin"} {
